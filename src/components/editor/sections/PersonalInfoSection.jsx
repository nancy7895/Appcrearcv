...
// =============================================================================
// Este componente maneja el formulario donde el usuario escribe su nombre,
// puesto, email, teléfono, redes sociales y sube su foto de perfil.
//
// ¿Cómo funcionan los formularios en React? (Concepto de "Inputs Controlados"):
// 1. Cada caja de texto tiene un `value` (el valor que lee de la memoria).
// 2. Cada vez que el usuario presiona una tecla, se dispara el evento `onChange`.
// 3. `onChange` llama a `updateActiveResume(...)`, lo que actualiza la memoria.
// 4. Al cambiar la memoria, la hoja de vista previa se actualiza al instante en pantalla.
...
export default function PersonalInfoSection({ isOpen, onToggle }) {
  ...
  // Función genérica para actualizar cualquier propiedad de personalInfo
  const handleChange = (field, value) => {
    updateActiveResume({
      personalInfo: {
        ...personalInfo,
        [field]: value // Reemplaza la clave específica (ej: fullName, email, etc.)
      }
    })
  }
  ...
  // ---------------------------------------------------------------------------
  // Usamos la API estándar `FileReader` del navegador para convertir el archivo de
  // imagen en un texto codificado en Base64 ("data:image/jpeg;base64,...").
  // Esto permite guardar la foto directamente en localStorage sin necesitar un servidor.
  const handleImageUpload = (e) => {
    const file = e.target.files?.[0]
    if (file) {
      // Validamos que la imagen no supere los 2MB para no saturar la memoria
  ...
  // Generador inteligente de resumen profesional simulado con IA
  const handleAiBio = () => {
  ...
  return (
    // Si isOpen es false, añadimos la clase CSS 'collapsed' para plegar el acordeón
    <div id="section-personal" className={`form-section ${!isOpen ? 'collapsed' : ''}`}>
      {/* Cabecera del acordeón: Al hacer clic llama a onToggle para abrir/cerrar */}
      ...
      <div className="form-section-body">
        ...
        <div className="avatar-upload-box">
          ...
          <div className="avatar-upload-info">
            <span style={{ fontSize: '0.82rem', fontWeight: '700', color: 'var(--text-primary)' }}>Foto de Perfil</span>
            <span style={{ fontSize: '0.74rem', color: 'var(--text-muted)' }}>PNG o JPG (Máx. 2MB). Idealmente cuadrada.</span>
        ...
        <div className="form-group">
          <div className="form-label">
            ...
            <button type="button" className="btn-ai-assist" onClick={handleAiBio}>
              <Sparkles size={11} /> Redactar con IA
            </button>
          </div>
          <textarea
